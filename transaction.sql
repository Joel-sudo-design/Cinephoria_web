...
VALUES
    (1, 2, 'Ghosted', 'Lorsque Cole tombe follement amoureux de la mystérieuse Sadie, il est loin de se douter qu''elle est en réalité un agent secret. Alors que Cole est bien décidé à revoir Sadie, ils sont soudain entraînés dans une aventure pour sauver le monde',
     @last_wednesday, @last_wednesday_plus6,
...
SELECT r.heure_debut, r.heure_fin, r.price, r.salle_id,
...
FROM (
    ...
    CROSS JOIN (
    SELECT 1 AS film_id UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4
...
WHERE DATE_ADD(CURDATE(), INTERVAL d.day_offset DAY) <= DATE(CONCAT(YEAR(CURDATE()), '-12-31'))
  AND NOT EXISTS (
    SELECT 1
    FROM seance s
    WHERE s.date = DATE_ADD(CURDATE(), INTERVAL d.day_offset DAY)
